...
switch (Answer.weekschedule) {
  ...
  case "Saturday":
    console.log(
      chalk.bold.bgRedBright(
        `Today you have two classes "Physics" & "PakStudies" class`
      )
    );
    console.log(chalk.bold.bgBlue(`This class conduct by Miss Humaiza `));
  ...
  case "Sunday":
    console.log(
      chalk.bold.bgRedBright(
        `Today is Sunday,your day off,but i have a class of IT at the Governor's house `
      )
    );
    console.log(
      chalk.bold.bgBlue(`Today my class will be conducted by Miss Hina Naseer`)
    );
